...
#' @param population_data A list representing the population, containing at least the
#'   `setting_sizes` and `<setting>_specific_riskiness` entries, as generated by
#'   `generate_population_data()`
#'
#' @return A named list with, for each setting in which an intervention is deployed, a 0/1
#'   vector of length equal to the number of locations in that setting, marking which
#'   locations received the intervention.
#'
#' @family intervention
#' @export
generate_intervention_switches <- function(parameters_list, population_data) {
  setting_types <- c("workplace", "school", "leisure", "household")

  if (isTRUE(parameters_list$intervention_joint_active)) {
    intervention_switches <- generate_joint_intervention_switches(
      parameters_list,
      population_data
    )
  } else {
...
generate_joint_intervention_switches <- function(parameters_list, population_data) {
  ...
  if (parameters_list[["intervention_joint_coverage_target"]] == "individuals") {
    setting_size_list <- list(
      "workplace" = setting_sizes$workplace,
      "school"    = setting_sizes$school,
      "leisure"   = setting_sizes$leisure
    )
  ...
  if (parameters_list[["intervention_joint_coverage_type"]] == "random") {
    ...
    riskiness_list <- list(
      "workplace" = population_data$workplace_specific_riskiness,
      "school"    = population_data$school_specific_riskiness,
      "leisure"   = population_data$leisure_specific_riskiness
    )
...
#'
#' @return A 0/1 vector of length equal to the number of locations in the setting.
#'
#' @family intervention
#' @export
...
#' install the intervention independently into one setting. A joint
#' intervention cannot be combined with setting-type specific interventions.
#'
...
#' field, and the intervention list and timestep are propagated to the
#' workplace, school and leisure slots, whose `_active` flags are switched on.
#'
#' @family intervention
#' @export
set_intervention_ach <- function(parameters_list,
  ...
  joint_setting_types <- c("workplace", "school", "leisure")
  setting_types <- c(joint_setting_types, "household")
  if (
    (setting == "joint" && any(unlist(parameters_list[paste0("intervention_", setting_types, "_active")]))) ||
      (setting != "joint" && isTRUE(parameters_list$intervention_joint_active))
  ) {
    stop("Joint interventions cannot be enabled together with setting-type specific interventions")
  }
  ...
  if (setting == "joint") {
    ...
    # Propagate the joint intervention list and timestep to each per-setting
    # slot, and switch each setting's _active flag on so the per-setting
    # efficacy calculation and FOI run through the same code path.
    for (s in joint_setting_types) {
      parameters_list[[paste0("intervention_", s, "_active")]]   <- TRUE
      parameters_list[[paste0("intervention_", s, "_list")]]     <- interventions
      parameters_list[[paste0("intervention_", s, "_timestep")]] <- timestep
    }
  }
...
#' the setting.
#' @param parameters_list A list of model parameters as generated by
#' [get_parameters()] (and populated by [set_intervention_ach()]).
#' @param setting Character. One of `"workplace"`, `"school"`, `"leisure"`,
#' or `"household"`.
#'
#' @param coverage_vector Optional 0/1 vector, of the same length as
#' `ach_values`, marking which locations are covered by the intervention (as
#' generated by [generate_intervention_switches()]). `NULL` (the default)
#' treats every location as covered.
#'
...
#' @family intervention
#' @export
calculate_efficacy_from_ach <- function(ach_values, parameters_list, setting, coverage_vector = NULL) {
  I    <- 1
