...
generate_population_data <- function(parameters_list, synthetic_population = NULL) {
  ...
  if (!is.null(synthetic_population)) {
    check_synthetic_population(synthetic_population, settings = rti_sampling)
  }
  if (rti_sampling) {
...
# and workplace, numbered 1, 2, ... with 0 meaning none.
...
check_synthetic_population <- function(synthetic_population, settings = FALSE) {
  ...
  columns <- c("household_id", "age", if (settings) c("school_id", "workplace_id"))
