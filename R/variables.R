...
#'
#' Households are sampled with replacement from `synthetic_population` when
#' given, and from the bundled San Francisco households
#' ([`baseline_household_demographics_usa`]) otherwise. Schools and workplaces
#' depend on the `school_workplace_sampling` parameter:
#'
#' * `"reference"`: school sizes are sampled from [`schools_usa`] and workplace
#'   sizes from a power-law distribution. Every child goes to school and every
#'   other adult to work.
#' * `"rti"`: individuals keep the school and workplace of the person they were
#'   sampled from, and school staff are drawn from working adults. This needs a
#'   `synthetic_population` with `school_id` and `workplace_id` columns. See
...
generate_population_data <- function(parameters_list, synthetic_population = NULL) {
  ...
  if (rti_sampling) {
    settings <- generate_rti_schools_and_workplaces(
      parameters_list = parameters_list,
      synthetic_population = synthetic_population,
      person_rows = household_age_list$person_rows,
...
#'   each individual's household and age class. When `synthetic_population` is
#'   given, it also has `person_rows`, giving the row of
#'   `synthetic_population$people` each individual was sampled from.
#'
#' @family variables
#' @export
generate_initial_households <- function(parameters_list, synthetic_population = NULL) {
  ...
  if (!is.null(synthetic_population)) {
    households$person_rows <- person_rows[rows]
  }
  households
}
...
#' `"rti"`. Each individual keeps the school and workplace of the person they
#' were sampled from. Households sampled more than once therefore share their
...
#' @param person_rows An integer vector giving the row of
#'   `synthetic_population$people` each individual was sampled from, as returned
#'   by [generate_initial_households()].
#' @param initial_age_classes A character vector assigning an age class to each individual
#'
...
generate_rti_schools_and_workplaces <- function(
  ...
  schools <- renumber_settings(synthetic_population$people$school_id[person_rows])
  workplaces <- renumber_settings(synthetic_population$people$workplace_id[person_rows])
