...
#' location and, when interventions are enabled, determines which locations are
#' covered and the resulting per-location efficacy.
#'
#' @param parameters_list A list of model parameters as generated by `get_parameters`
#' @export
generate_population_data <- function(parameters_list) {
  ...
  # If any intervention is active (per-setting or joint), determine which
  # locations of each setting have it deployed.
  if (
    any(
    ...
    data$intervention_switches <- generate_intervention_switches(parameters_list, data)
  }

  # Calculate location-specific efficacy from ACH using W-R for each setting
  # with an active intervention. The coverage vector is used inside
  # calculate_efficacy_from_ach to zero out delta for uncovered locations.
  for (setting in c("workplace", "school", "leisure", "household")) {
    if (isTRUE(parameters_list[[paste0("intervention_", setting, "_active")]])) {
      data[[paste0(setting, "_specific_efficacy")]] <- calculate_efficacy_from_ach(
        ach_values = data[[paste0(setting, "_specific_ach")]],
        parameters_list = parameters_list,
        setting = setting,
        coverage_vector = data$intervention_switches[[setting]]
      )
    }
  }
