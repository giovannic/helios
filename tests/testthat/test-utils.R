test_that("get_setting_size() returns integer vectors for setting as workplace, school, and household", {
  ...
  x <- create_variables(parameters_list, generate_population_data(parameters_list))
  testthat::expect_vector(
    get_setting_size(x, setting = "workplace"),
    ptype = integer()
  )
  testthat::expect_vector(
    get_setting_size(x, setting = "school"),
    ptype = integer()
  )
  testthat::expect_vector(
    get_setting_size(x, setting = "household"),
    ptype = integer()
  )
})
...
test_that("get_setting_size() returns integer vectors for setting as workplace, school, and household", {
  ...
  testthat::expect_error(get_setting_size(x, setting = 1))
})
